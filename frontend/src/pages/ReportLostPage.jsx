...
import { CATEGORIES, LOCATIONS, MAX_IMAGE_SIZE_MB, ALLOWED_IMAGE_TYPES } from '../utils/constants'
...
export default function ReportLostPage() {
  ...
  const [step,         setStep]         = useState('form') // 'form' | 'matching' | 'done'
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // 2. Save item to database via backend
      ...
      // 3. Run smart match in background
      setStep('matching')
      toast.loading('Searching for matches…', { id: 'match' })
      try {
        await runAndPersistMatches(created.data.id, token)
      } catch (_) { /* matches are best-effort */ }
      toast.dismiss('match')

      setStep('done')
      toast.success('✅ Lost item reported! Matches are being scanned.')
    } catch (err) {
      toast.dismiss('upload')
      toast.dismiss('save')
      toast.dismiss('match')
    ...
    return (
      <div className="form-page">
        <div className="text-center py-16">
          ...
          <h2 className="text-2xl font-extrabold text-[#1e1b4b] mb-3">Lost Item Reported!</h2>
          <p className="text-[#6b7280] mb-8 max-w-md mx-auto">
            Your report has been saved. Our matching engine is scanning found reports for possible matches.
            You will be notified if we find something!
          </p>
  ...
  return (
    <div className="form-page">
      <div className="form-header">
        ...
        <p>Fill in the details below. Upload a photo for better AI matching.</p>
      </div>
      ...
      <form className="item-form" onSubmit={handleSubmit} noValidate>
        <div className="form-grid">
          ...
          <Input label="Item Name"             required value={form.itemName}    onChange={set('itemName')}    placeholder="e.g. Black Wallet"      error={errors.itemName} />
          ...
          <Input label="Color" required value={form.color} onChange={set('color')} placeholder="e.g. Black, Blue" error={errors.color} />

          <Select label="Lost Location" required value={form.location} onChange={set('location')} error={errors.location}>
            <option value="">-- Select Location --</option>
            {LOCATIONS.map(l => <option key={l} value={l}>{l}</option>)}
          </Select>

          <Input label="Lost Date" type="date" required value={form.date} onChange={set('date')} max={TODAY} error={errors.date} />
        ...
        <Textarea
          ...
          placeholder="Describe the item – unique marks, brand, contents, stickers, etc."
          error={errors.description}
          containerClass="full-width mb-4"
        />

        <div className="form-actions">
          <Button type="submit" variant="danger" size="lg" loading={uploading}>
            🔴 Report Lost Item
          </Button>
        </div>
      </form>
